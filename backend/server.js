...
app.get('/api/status', async (req, res) => {
  try {
    ...
    // Убираем проверку Python сервиса из статуса - она не нужна здесь
    // const pythonServiceStatus = await checkPythonService();

    res.json({
      status: 'ok',
      // python_service: pythonServiceStatus ? 'running' : 'stopped',
...
const cleanupAndExit = async () => {
  ...
  isShuttingDown = true;
  console.warn('Начинаем корректное завершение работы...');
  
  try {
    // Закрываем HTTP сервер
    if (server) {
      server.close(() => {
        console.warn('HTTP сервер остановлен');
      });
    }
    ...
    console.warn('Завершение работы завершено');
    process.exit(0);
  } catch (error) {
...
process.on('SIGINT', () => {
  console.warn('Получен сигнал SIGINT, останавливаем сервер...');
  cleanupAndExit();
});

process.on('SIGTERM', () => {
  console.warn('Получен сигнал SIGTERM, останавливаем сервер...');
  cleanupAndExit();
});

// Обработка ошибок Python сервиса
process.on('uncaughtException', (error) => {
  if (error.message.includes('ECONNREFUSED') || error.message.includes('Python')) {
    console.warn('Ошибка Python сервиса, Node.js продолжает работать');
        setTimeout(() => {
      console.warn('Перезапуск сервера через 5 секунд...');
            process.exit(1);
        }, 5000);
  } else {
...
process.on('unhandledRejection', (reason, promise) => {
  if (reason.message && (reason.message.includes('ECONNREFUSED') || reason.message.includes('Python'))) {
    console.warn('Ошибка Python сервиса, Node.js продолжает работать');
    setTimeout(() => {
      console.warn('Перезапуск сервера через 5 секунд...');
      process.exit(1);
...
server.listen(PORT, () => {
  console.warn(`Node.js сервер запущен на порту ${PORT}`);
  console.warn(`Python сервис камер будет доступен на порту 5000`);
});
